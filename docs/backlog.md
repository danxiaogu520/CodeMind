...
| CM-509 | P1 | 真实仓库与 Agent baseline 评测 | CM-504 | 固定 SHA；比较 grep/read-top-k、Hybrid 和 Hybrid+Graph；逐任务失败可审计 |
| CM-510 | P1 | 多跳与影响面评测 | CM-509 | 分开评估锚点与邻居；并列报告 graph-derived、co-change、curated 真值及 Recall/Precision/F1 |
| CM-511 | P1 | Token/成本基线校准 | CM-509 | 区分全仓库、Agent 和变更文件 baseline；正式报告使用目标 tokenizer 并记录延迟/费用 |
...
| CM-702 | P1 | 影响面分析与变更方案工作流 | CM-407, CM-701, CM-707, CM-708 | 给出可验证依赖路径、透明风险因子与测试建议，不自动写代码 |
...
| CM-707 | P1 | 版本化有界图遍历原语 | CM-305, CM-407 | 支持方向、边类型、深度、路径、置信度、预算、分页/截断和确定排序 |
| CM-708 | P1 | Diff 行范围到符号映射 | CM-701, CM-707 | 新增、修改、删除、重命名均绑定基准/目标版本并输出未解析警告 |
| CM-709 | P2 | 入口 Flow 派生视图 | CM-707, CM-510 | 入口、关键路径、测试和来源关系可追溯，更新后精确失效 |
| CM-710 | P2 | Community 与架构投影评估 | CM-509, CM-707 | 仅在真实评测优于目录基线时引入；固定 seed，保留确定性降级 |
| CM-711 | P2 | MCP 图能力适配器 | CM-305, CM-707 | 复用 application port、认证和配额，不直连数据库，安装支持 dry-run |

Epic H 的图分析任务应遵循 [`code-review-graph` 借鉴分析与实施映射](reference-code-review-graph.md)：借鉴工作流和评测方法，不迁移其单机存储架构或自动修改能力。
